...
module mkI2c{label}Tb(Empty);
  ...
  rule wire_;
    ...
    Bit#(1) sda = (mp == 1 || sp == 1) ? 0 : 1;
    Bit#(1) scl = (d.pins.scl_pull == 1) ? 0 : 1;
    d.pins.sda_in(sda);
    d.pins.scl_in(scl);
    sclPrv <= scl;
  ...
  rule wrWait (ph == WrWait);
    let x <- d.regs.access(RegReq {{ addr: rSTAT, write: False,
                                     wdata: 0, wstrb: 4'hF }});
    // busy 落下且 irqf 抬起来才算这一字节走完
    if (x.rdata[6] == 0 && x.rdata[0] == 1) begin ph <= WrCheck; s <= 0; end
  endrule
  ...
  rule rdWait (ph == RdWait);
    let x <- d.regs.access(RegReq {{ addr: rSTAT, write: False,
                                     wdata: 0, wstrb: 4'hF }});
    if (x.rdata[6] == 0 && x.rdata[0] == 1) begin ph <= RdCheck; s <= 0; end
  endrule
  ...
  rule fin (ph == Done);
    ...
    else $display("PASS i2c: a byte out on the wire, a byte in, and the ack is sent");
    $finish(bad ? 1 : 0);
  endrule
endmodule
